...
FIXTURE = Path(__file__).resolve().parents[3] / "analog" / "fixtures" / "sdg_machine_small.h5"


def test_audit_machine_fixture() -> None:
    ...
    assert desc.tags.get("layout_kind") == "sdg_machine"
    assert (desc.tags.get("seeAlso") or "").endswith("HostMachine")
